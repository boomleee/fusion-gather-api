...
export class CreateEntities1705398782380 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`eventLocation\` (\`id\` int NOT NULL AUTO_INCREMENT, \`address\` varchar(255) NOT NULL, \`latitude\` decimal(17, 14) NOT NULL, \`longitude\` decimal(17, 14) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`event\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`date\` datetime NOT NULL, \`capacity\` int NOT NULL, \`ticketPrice\` int NOT NULL, \`isFree\` tinyint NOT NULL, \`status\` tinyint NOT NULL, \`description\` varchar(255) NOT NULL, \`userId\` int NULL, \`qrcodeId\` int NULL, \`eventLocationId\` int NULL, UNIQUE INDEX \`REL_01cd2b829e0263917bf570cb67\` (\`userId\`), UNIQUE INDEX \`REL_7812679849c61fb1dde3552e01\` (\`qrcodeId\`), UNIQUE INDEX \`REL_45e8e0655f148c8ae4dd9d0cc8\` (\`eventLocationId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`boothLocation\` (\`id\` int NOT NULL AUTO_INCREMENT, \`latitude\` decimal(17, 14) NOT NULL, \`longitude\` decimal(17, 14) NOT NULL, \`boothId\` int NULL, \`eventLocationId\` int NULL, UNIQUE INDEX \`REL_78523cc4fa53616a5e02838ba2\` (\`boothId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`boothLocation\` ADD CONSTRAINT \`FK_78523cc4fa53616a5e02838ba27\` FOREIGN KEY (\`boothId\`) REFERENCES \`booth\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`boothLocation\` ADD CONSTRAINT \`FK_6af25cabf7184ca1f6d7fed7500\` FOREIGN KEY (\`eventLocationId\`) REFERENCES \`eventLocation\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }
